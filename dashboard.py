...
# 3. UNLOCK SECRETS & CONNECT TO DATABASE
load_dotenv()
...
# Native Sidebar Layout
with st.sidebar:
    # Sidebar Header with Image Icon aligned left
    st.markdown(
        f"""
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 5px;">
            {'<img src="' + logo_uri + '" style="width: 32px; height: 32px; object-fit: contain;" />' if logo_uri else '<span style="font-size: 1.6rem;">📦</span>'}
            <h2 style="margin: 0; padding: 0; font-size: 1.4rem; font-weight: 800; color: var(--text-color);">InventoryLink</h2>
        </div>
        """,
        unsafe_allow_html=True
    )
    st.caption("Store operations dashboard")
    st.divider()
    
    st.write("**Dashboard Controls**")
    st.caption("Refine reports by date, category, and item.")
    ...
    st.divider()
    st.caption(f"**Store ID:** {current_store_id}")
...
# Styled HTML Hero Banner (Replaces standard st.title)
st.markdown(
    f"""
    <div class="hero-banner">
        <h1>{store_name} Dashboard</h1>
        <p>Inventory, sales, credit, and cash drawer health for <strong>{today.strftime('%B %d, %Y')}</strong>.</p>
    </div>
    """,
    unsafe_allow_html=True
)
...
with overview_tab:
    ...
    with chart_col:
        ...
        if filtered_inv.empty:
            ...
            st.plotly_chart(style_fig(fig, 360), use_container_width=True, theme="streamlit")
    ...
    with table_col:
        ...
        if filtered_inv.empty:
            ...
            if attention_df.empty:
                st.success("No low-stock items in this view.", icon="✅")
            else:
                st.dataframe(
...
with inventory_tab:
    ...
    if filtered_inv.empty:
        ...
        with inv_right:
            ...
            st.plotly_chart(style_fig(fig, 380), use_container_width=True, theme="streamlit")

        st.divider()
        low_stock = filtered_inv[filtered_inv["quantity"] <= 5].sort_values("quantity")
        section_title("Low and Out-of-Stock")
        if low_stock.empty:
            st.success("All filtered items have more than 5 pieces in stock.", icon="✅")
        else:
            st.dataframe(
    ...
    if df_sales.empty:
        ...
        if filtered_sales.empty:
            ...
            with sales_left:
                ...
                st.plotly_chart(style_fig(fig, 390), use_container_width=True, theme="streamlit")
            ...
            with sales_bottom_left:
                ...
                st.plotly_chart(style_fig(fig, 320), use_container_width=True, theme="streamlit")
...
with credit_tab:
    st.caption("Credit rows are queried fresh so account status stays synchronized with Telegram actions.")
    ...
    if not credit_accounts:
        ...
        with credit_left:
            ...
            if bucket_df.empty:
                ...
                st.plotly_chart(style_fig(fig, 350), use_container_width=True, theme="streamlit")
